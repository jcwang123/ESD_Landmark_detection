...
from datasets.esd_v2 import ESD_v2,ESD_v2_eval
...
parser.add_argument('--dataset', type=str, default='esd_v2', choices=['coco', 'pascal', 'esd', 'esd_v2'])
parser.add_argument('--arch', type=str, default='large_hourglass')
parser.add_argument('--load_model', type=str, default='/raid/wjc/code/ESD/CenterNet/models/ctdet_coco_dla_1x.pth')
...
#   num_gpus = cfg.num_gpus
  ...
  if cfg.dataset == 'esd':
    Dataset = ESD
    Dataset_eval = ESD_eval
  else:
    Dataset = ESD_v2
    Dataset_eval = ESD_eval
  train_dataset = Dataset(cfg.data_dir, 'train', split_ratio=cfg.split_ratio, img_size=cfg.img_size, scale=cfg.scale, shift=cfg.shift)
  ...
  val_dataset = Dataset_eval(cfg.data_dir, 'test', test_scales=[0.5], test_flip=False)
  ...
  if 'hourglass' in cfg.arch:
    ...
    model = get_pose_net(num_layers=int(cfg.arch.split('_')[-1]), num_classes=val_dataset.num_classes)
  elif 'dla' in cfg.arch:
    from nets.dla import get_pose_net as dla
    model = dla(num_layers=int(cfg.arch.split('_')[-1]), num_classes = val_dataset.num_classes)
  else:
...
#     model = load_model(model, cfg.pretrain_dir)
  ...
  def val_map(epoch):
    ...
    print(eval_results)
    summary_writer.add_scalar('val_mAP/mAP', eval_results[1], epoch)
