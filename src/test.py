...
parser.add_argument('--root_dir',
                    type=str,
                    default='/raid/wjc/logs/landmark_detection')
parser.add_argument('--data_dir',
                    type=str,
                    default='/raid/wjc/code/ESD/Landmark_Detection/data')
...
parser.add_argument('--test_scales', type=str,
                    default='0.5')  # 0.5,0.75,1,1.25,1.5
...
def main():
    ...
    val_dataset = landmarks_eval(cfg.data_dir,
                                 'test',
                                 fold=cfg.fold,
    ...
    if 'dla' in cfg.arch:
        from nets.dla import get_pose_net as dla
        model = dla(num_layers=int(cfg.arch.split('_')[-1]),
                    num_classes=2 if cfg.use_relation else 1)
    else:
        raise NotImplementedError
    ...
    with torch.no_grad():
        ...
        for inputs in val_loader:
            ...
            for scale in inputs:
                ...
                dets = ctdet_decode(hmap,
                                    regs,
                                    w_h_,
                                    K=cfg.test_topk,
                                    compute_edge_reg=False)
