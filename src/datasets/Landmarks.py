...
class landmarks(data.Dataset):
    def __init__(self,
        ...
        if split == 'test':
            self.annot_path = os.path.join(self.data_dir, 'annotations_5folds',
                                           'fold{}'.format(fold), 'test.json')
        elif split == 'train':
            self.annot_path = os.path.join(self.data_dir, 'annotations_5folds',
                                           'fold{}'.format(fold), 'train.json')
        elif split == 'infer':
            self.annot_path = os.path.join(self.data_dir, 'all.json')
        else:
            self.annot_path = os.path.join(self.data_dir, 'annotations_5folds',
                                           'fold{}'.format(fold), 'valid.json')
    ...
    def __getitem__(self, index):
        ...
        if self.split == 'train':
            ...
            center[0] = np.random.randint(
                low=int(center[0] * (1 - self.shift)),
                high=int(center[0] * (1 + self.shift)))
            center[1] = np.random.randint(
                low=int(center[1] * (1 - self.shift)),
                high=int(center[1] * (1 + self.shift)))
...
#         -----------------------------------debug---------------------------------
        ...
        if self.num_classes == 2:
            relation_points = np.array(get_relation_points(
                bboxes, self.center),
                                       dtype='int32')
            #             if len(bboxes)==1:
...
class landmarks_eval(landmarks):
    def __init__(self,
                 ...
                 fix_size=False,
                 img_size=(512, 512)):
        super(landmarks_eval, self).__init__(data_dir,
                                             split,
                                             fold,
                                             use_relation=False,
                                             use_center=False)
        ...
        for img_id, sample in batch:
            out.append((img_id, {
                s: {
                    ...
                    if k == 'image' else np.array(sample[s][k])
                    for k in sample[s]
                }
                for s in sample
            }))
