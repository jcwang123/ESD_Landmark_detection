...
import sys
sys.path.append('/raid/wjc/code/ESD/simple_centernet/')
...
class DLASeg(nn.Module):
    def __init__(self, base_name, down_ratio, final_kernel,
                 last_level, head_conv, out_channel=0, num_classes=1, pretrained = True):
...
#         print(base_name)
    ...
    def forward(self, x):
        ...
        out = [[self.hmap(y[-1]), self.regs(y[-1]), self.w_h_(y[-1])]]
        return out

def get_pose_net(num_layers, head_conv=256, down_ratio=4, num_classes=1):
...
if __name__ == '__main__':
  ...
  net = get_pose_net(34, num_classes=1).cuda()
  ...
  model = load_model(net,'/raid/wjc/code/ESD/CenterNet/models/ctdet_coco_dla_1x.pth')